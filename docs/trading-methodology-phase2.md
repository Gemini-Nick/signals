...
这个理论解释了三个经典现象：
1. **周末大热必死**：两天信息发酵把所有人洗成同一方向，周一开盘没有对手盘
2. **懂王反复溜市场**：每次喊话制造瞬间策略趋同，流动性蒸发，波动就是利润
3. **AI时代更脆弱**：信息泛滥+分析框架趋同→观点多样性塌缩→市场更脆弱

#### 后端

**文件：** `signals/layers/market_context.py`

新增 `ConsensusRisk` 数据类 + `detect_consensus_risk()` 函数。

**三个维度：**
- 趋势一致性（40%）：多少指数趋势方向相同
- 信号一致性（30%）：买/卖信号是否集中在同一侧
- 情绪极端度（30%）：情绪是否处于极端状态（亢奋/恐慌）
...
class ConsensusRisk:
    level: str        # "高" / "中" / "低"
    score: float      # 0-100
    direction: str    # "偏多共识" / "偏空共识" / ""
    detail: str       # 人类可读描述
    warning: str      # 操作建议
```

**判定阈值：**
- 高 (≥70)：策略高度趋同，对手盘稀缺，反转概率高
- 中 (≥45)：观点偏集中，留意流动性变化
- 低 (<45)：观点分歧健康，流动性充裕
...
`score_signals()` 新增 `consensus_risk_level` 参数，共识风险高时信号打折：
```python
_CONSENSUS_MULT = {"高": 0.75, "中": 0.90, "低": 1.00}
```

逻辑：当所有人看同一方向时，顺势信号的可靠性降低（对手盘不足，追涨杀跌风险高）。

#### 决策简报集成

共识风险高时自动生成操作建议：
- 偏多共识 → "多头拥挤，谨防获利盘踩踏→控制仓位，勿追高"
- 偏空共识 → "空头拥挤，恐慌充分释放后反弹猛烈→准备抄底清单"
...
```
🚨 共识风险: 高(78/100) 偏多共识
   90%指数趋势多头一致，信号集中在买侧，情绪亢奋
   ⚠️ 多头拥挤，对手盘稀缺，谨防获利盘踩踏→控制仓位，勿追高
```
