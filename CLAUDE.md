...
1. **先尝试内置技能**：运行 `python scripts/wechat_run.py "<消息原文>"`
   - 退出码 0 = 技能匹配成功，直接输出结果文本
   - 退出码 1 = 无匹配技能，进入步骤 2
2. **无匹配时自行回答**：用你自己的知识回答用户问题，保持简洁（微信消息不宜过长）

### 内置技能触发词

| 技能 | 触发词 | 示例 |
|------|--------|------|
| 个股分析 | 分析、看看、诊股 | 「分析 茅台」「看看 600519」 |
| 回测 | 回测、backtest | 「回测 600519 日线」 |
| 大盘研判 | 大盘、指数、市场 | 「大盘怎么样」 |
| 行业排行 | 行业、板块、排行 | 「行业排行」 |
| 热门概念 | 热点、主题、概念 | 「今日热点」 |
| 社交舆情 | 舆情、热度、人气 | 「舆情 宁德时代」 |
| 盘前计划 | 计划、盘前 | 「盘前计划」 |
| 周策略 | 周策略、本周 | 「周策略」 |
| 帮助 | 帮助、help、? | 「帮助」 |
