...
| `CLAUDE.md` | Agent 行为指令 — 描述可用工具 + 意图判断示例，CC 自己决策 |
| `scripts/wechat_run.py` | 工具入口 — 按工具名执行，不做意图理解 |
| `signals/wechat/skills.py` | 工具实现 — industry_ranking() + review() 两个函数 |

### 设计思路：CC 是大脑，工具是手脚

```
                    ┌──────────────┐
                    │  分析引擎     │  core/ + layers/ + data/
                    └──────┬───────┘
                           │
         ┌─────────────────┼─────────────────┐
         ▼                 ▼                  ▼
  ┌──────────────┐  ┌─────────────┐   ┌──────────────┐
  │  Web (API+UI) │  │  工具脚本    │   │  CC 自由发挥   │
  │  给人看        │  │  预制管道    │   │  读代码+跑代码  │
  └──────────────┘  └─────────────┘   └──────────────┘
                           ▲                  ▲
                           │                  │
...
| 角色 | 职责 | 不做什么 |
|------|------|----------|
| **CC (Claude Code)** | 理解意图、选择工具、兜底回答 | — |
| **工具脚本 (wechat_run.py)** | 执行 CC 指定的工具 | 不理解意图、不做关键词匹配 |
| **分析引擎 (signals/)** | 算数据、跑分析 | 不知道微信的存在 |
| **Web UI** | 给人看图表 | 微信 Agent 不依赖它 |

**为什么工具脚本不做意图理解？**

CC 本身就是最好的意图理解层。"行业" 可能是要排行榜，可能是问某个具体行业，可能是问轮动阶段 — 只有 CC 能区分。工具只管执行，不抢 CC 的活。
