# -*- coding: utf-8 -*-
"""Repair HK daily price mismatches against Tencent HK website history.

This script is deliberately narrower than the structural OHLC repair:
it only rewrites HK daily bars when Tencent has a same-date row and the
local volume is compatible with Tencent's volume.  MongoDB time-series
documents are replaced by backing up originals, deleting the affected
key/date documents, then inserting corrected copies.
"""
...
from signals.sync.modules.hk_stock_daily import _normalize_ohlc_bounds, _pure_hk_code


MONGO_URL = "mongodb://127.0.0.1:27017/signals"
DB_NAME = "signals"
BACKUP_PREFIX = "bars_hk_daily_tencent_price_backup"
...
def _parse_symbols(raw: str) -> list[str]:
    ...
    for item in str(raw or "").replace(";", ",").split(","):
        ...
        if value.startswith("HK."):
            symbols.append(f"HK.{_pure_hk_code(value)}")
        elif value.isdigit():
            symbols.append(f"HK.{value.zfill(5)}")
...
def _online_row_values(row) -> dict[str, float | None]:
    ...
    if prices is None:
        prices = {"open": None, "high": None, "low": None, "close": None}
    return {**prices, "vol": _num(row.get("成交量"))}
...
def _corrected_doc(original: dict[str, Any], online: dict[str, float | None]) -> dict[str, Any]:
    ...
    meta["source"] = "website_tencent_hk_price_repair"
    meta["source_price"] = "website_tencent_hk"
    corrected["meta"] = meta
    corrected["source"] = "website_tencent_hk_price_repair"
    for field in PRICE_FIELDS:
...
def _symbol_rows(db, args: argparse.Namespace) -> list[dict[str, Any]]:
    match: dict[str, Any] = {"meta.market": "HK", "meta.freq": "日线"}
...
def _docs_for_symbol(db, symbol: str, args: argparse.Namespace) -> list[dict[str, Any]]:
    match: dict[str, Any] = {"meta.market": "HK", "meta.freq": "日线", "meta.symbol": symbol}
    dt_filter: dict[str, Any] = {}
    date_from = _date_from_cli(args.date_from)
    date_to = _date_from_cli(args.date_to)
    if date_from:
...
def _fetch_hk_daily_with_retries(symbol: str, args: argparse.Namespace):
    errors = 0
    last_exc: Exception | None = None
    attempts = max(1, args.provider_retries + 1)
    for attempt in range(1, attempts + 1):
        try:
            df, source_url = _website_tencent_hk_daily(symbol, count=args.tencent_count, timeout=args.provider_timeout)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        "mode": "hk_daily_price_mismatch_from_tencent",
        "apply": args.apply,
    ...
    print(
        json.dumps(
            {"event": "hk_daily_price_repair_start", "apply": args.apply, "selected_symbols": len(symbol_rows)},
            ensure_ascii=False,
        ),
        flush=True,
    )
    ...
    for idx, item in enumerate(symbol_rows, start=1):
        ...
        try:
            df, source_url, retry_errors = _fetch_hk_daily_with_retries(symbol, args)
        ...
        for doc in docs:
            ...
            local_vol = _num(doc.get("vol"))
            online_vol = _num(online.get("vol"))
            if args.require_volume_match and not _volume_matches(local_vol, online_vol, args):
            ...
            if args.apply:
                backup_doc = dict(doc)
                backup_doc["repair"] = {
                    "type": "hk_daily_price_mismatch_from_tencent",
        ...
        if args.progress_every and (idx % args.progress_every == 0 or summary["planned"] >= args.max_docs > 0):
            print(
                json.dumps(
                    {
                        "event": "hk_daily_price_repair_progress",
                        "symbols": idx,
    ...
    print(json.dumps({"event": "hk_daily_price_repair_done", **summary}, ensure_ascii=False, default=_json_default), flush=True)
    client.close()
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--symbols", default="", help="Comma list such as HK.00073,00508.")
    parser.add_argument("--sources", default="", help="Optional source filter, e.g. akshare_stock_hk_hist.")
    ...
    parser.add_argument("--max-docs", type=int, default=0, help="Stop after this many planned repair docs; 0 means no limit.")
    ...
    parser.add_argument("--output", default="/tmp/signals_hk_daily_price_mismatch_from_tencent.json")
    return parser
