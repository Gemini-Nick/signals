...
class MarketContext:
    ...
    def to_text(self) -> str:
        ...
        lines.append(f"  {dir_emoji} 综合: {self.overall_direction}  |  风格: {self.growth_vs_value}{phase_str}{div_str}")
    ...
    def to_feishu_text(self) -> str:
        ...
        lines.append(f"综合: {self.overall_direction}  风格: {self.growth_vs_value}{phase_str}")
        lines.append("─" * 36)
    ...
    def to_feishu_card(self, perf_summary: dict = None,
        ...
        phase_emoji = _SENTIMENT_EMOJI.get(
            SentimentPhase(self.sentiment_phase), "⚪") if self.sentiment_phase != "未知" else ""
        phase_str = f"  |  {phase_emoji}**{self.sentiment_phase}**" if self.sentiment_phase != "未知" else ""
        l1_lines.append(f"{dir_emoji} **综合: {self.overall_direction}**  |  风格: {self.growth_vs_value}{phase_str}")
