...
def _strategy_rule_views(docs: list[dict[str, Any]], now: datetime) -> list[dict[str, Any]]:
    ...
    views.append({
        ...
        "participation_rule": "硬技术触发是候选前提；产业链和知识库只能确认、降级、提示暂不参与或提供背景。",
        "risk_rule": "公开利好不等于可交易，必须看赔率、拥挤度、参与性和风报比。",
        "right_side_requirement": "趋势弱或阶段不明时不做左侧，盘中等待止跌、放量、5m/15m 下单确认。",
    ...
    if daozhang_text or _has_any(combined, ("道长", "国证2000", "超大盘", "科创50")):
        views.append({
            ...
            "participation_rule": "板块观点只做背景，个股必须回到硬技术和买点确认。",
    ...
    if pangge_text or _has_any(combined, ("胖哥", "右侧", "恒科", "风报比")):
        views.append({
            ...
            "participation_rule": "先确认参与条件，再谈方向；关键位、跌破与否、买点确认和风报比一起判断。",
            "risk_rule": "趋势不站队时等待止跌、放量或买点确认；左侧冲动默认降级。",
            "right_side_requirement": "必须等 5m/15m 下单确认、止跌或放量，不用盘中猜方向。",
