    ...
    *,
    ...
    db["data_freshness"].update_one(
        ...
        {"$set": {
            ...
            "date_key": date_key,
            "updated_at": now,
            "stale_reason": "" if count > 0 and not freshness_override else (error or "etf_spot_empty"),
            "count": count,
            "mirrored": mirrored,
...
def sync_etf_spot_snapshot(db: Database, proxy_url: str = None) -> dict:
    ...
    immutable_result = append_snapshot_docs(
        db, source_id="signals-etf-spot", trade_date=trade_date, docs=docs
    )
    inserted = 0
    modified = 0
    mirrored_inserted = 0
