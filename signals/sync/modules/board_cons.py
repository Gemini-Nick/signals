...
def _eastmoney_board_code_map(kind: str) -> dict[str, str]:
    ...
    return {
        _text(row.get("f14")): _text(row.get("f12"))
        for row in rows
        if _text(row.get("f14")) and _text(row.get("f12"))
    }
...
def _eastmoney_delay_cons(name: str, *, kind: str):
    ...
    if not code:
        raise KeyError(f"eastmoney_board_code_missing:{kind}:{name}")
    rows = _eastmoney_delay_clist({
        "pn": "1",
        "pz": "5000",
        "po": "1",
...
def _fetch_board_cons(board_name: str, proxy_url: str = None):
    ...
    try:
        ...
        try:
            source = "ths"
            return _call_provider(lambda: ak.stock_board_industry_cons_ths(symbol=board_name)), source
...
def _fetch_concept_cons(concept_name: str, proxy_url: str = None):
    ...
    try:
        ...
        try:
            fetch_ths = getattr(ak, "stock_board_concept_cons_ths")
            source = "ths"
